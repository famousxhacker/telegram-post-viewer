...
def tui():
    while True:
        print(logo)
        print(f'''
  [ Channel ]: {channel.capitalize()}/{post}
  [ Views ]: {real_views}

  [ Conections Errors ]: {proxy_errors}
  [ Tokens Errors ]: {token_errors}
...
print(logo)
channel, post = input(' [ INPUT ] Link of the post: ').replace(
    'https://t.me/', '').split('/')
...
try:
    ...
    print(' [ OUTPUT ]  Wait a few seconds to execute the threads...')
